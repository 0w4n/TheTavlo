...
async function assertAccess(ctx: any, scope: { ownerId: string; panelId: string }, write = false) {
  ...
  if (write && data.role !== "editor") throw new TRPCError({ code: "FORBIDDEN", message: "Solo el dueño o un editor pueden modificar tareas." });
}

function collection(ctx: any, scope: { ownerAccountType: AccountType; ownerId: string; panelId: string }) {
  return ctx.db.collection(`${scope.ownerAccountType}/${scope.ownerId}/panels/${scope.panelId}/tasks`);
}
