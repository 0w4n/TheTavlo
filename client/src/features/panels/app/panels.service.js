...
import { ok, err, notFoundErr, isErr, validationErr, } from "#core/appCore/domain/AppCore.type";
export class PanelsService {
    ...
    async resolveChain(panelIds) {
        ...
        if (!validation.valid) {
            ...
            return err(validationErr(`El panel "${validation.panelId}" no está anidado correctamente en la cadena de la URL`, { panelIds: "La cadena de paneles en la URL no es válida" }));
        }
        return ok(panelIds.map((id) => panelsById.get(id)));
    }
    // ─── Mutaciones ──────────────────────────────────────────────────────────
    async createPanel(data, parentId) {
    ...
    async addSubPanel(parentRef, childRef) {
        return this.repository.addSubPanel(parentRef, childRef);
    }
    async archivePanel(id) {
        return this.repository.archive(id);
    }
    async unarchivePanel(id) {
    ...
    async deletePanel(id) {
        ...
        if (!PanelRules.canDelete(panel)) {
            return err(validationErr("No se puede eliminar el panel por defecto", {
