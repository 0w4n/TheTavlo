...
import {
  ...
  refTo,
} from "../infraestructure/__fixtures__/fakePanelRepository";
...
function makeDefaultPanel(id: string): Panel {
  return {
    id,
    parentId: null,
    name: "",
    color: -1,
    icon: "",
    sharedWith: undefined as unknown as null,
    createdAt: now,
    updatedAt: now,
  };
}
...
describe("PanelsService.resolveChain", () => {
  it("resuelve una cadena válida de N niveles", async () => {
    ...
    if (isOk(result)) expect(result.value.map((p) => p.id)).toEqual(["a", "b", "c"]);
  });
  ...
  it("rechaza con NotFound si un id de la cadena no existe", async () => {
    ...
    if (isErr(result)) expect(result.err.kind).toBe("NotFound");
  });
  ...
  it("rechaza con Validation si el anidamiento no coincide con la base de datos", async () => {
    ...
    expect(isErr(result)).toBe(true);
    if (isErr(result)) expect(result.err.kind).toBe("Validation");
  });
