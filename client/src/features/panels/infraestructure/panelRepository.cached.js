...
import { deleteCachedPanel, deleteCachedPanels, ensurePanelsInCache, fetchChildrenPanels, getCachedPanel, invalidateChildren, setCachedPanel, setCachedPanels, } from "./panelsCache";
/**
 ...
 */
export class CachedPanelsRepository {
    ...
    async findAll() {
        ...
        if (isOk(result))
            setCachedPanels(this.cacheKey, result.value);
        return result;
    }
    async findHomePanel() {
    ...
    async findManyByIds(ids) {
        return ensurePanelsInCache(this.cacheKey, ids, (missing) => this.inner.findManyByIds(missing));
    }
    async findByRef(ref) {
    ...
    async findByParentId(parentId) {
        return fetchChildrenPanels(this.cacheKey, parentId.id, () => this.inner.findByParentId(parentId));
    }
    async findDocRef(id) {
    ...
    async addSubPanel(parentRef, childRef) {
        ...
        if (isOk(result))
            invalidateChildren(this.cacheKey, parentRef.id);
        return result;
    }
