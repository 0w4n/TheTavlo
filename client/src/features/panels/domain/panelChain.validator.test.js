...
function makePanel(id, parentId) {
    return {
        id,
...
describe("validatePanelChain", () => {
    ...
    it("acepta una cadena correctamente anidada de varios niveles", () => {
        ...
        expect(validatePanelChain(["a", "b", "c", "d"], indexById([a, b, c, d]))).toEqual({ valid: true });
    });
    ...
    it("rechaza cuando el panel intermedio no coincide (URL combina dos ramas distintas del árbol)", () => {
        ...
        expect(validatePanelChain(["a", "b", "y"], indexById([a, b, x, y]))).toEqual({ valid: false, reason: "broken-link", panelId: "y" });
    });
