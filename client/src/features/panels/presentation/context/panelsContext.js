...
import { ReturnType, } from "./panelsContext.types";
import { isErr, isOk, unexpectedErr, err, notFoundErr, } from "#core/appCore/domain/AppCore.type";
export const PanelsContext = createContext(undefined);
export function PanelsProvider({ children, panelsService, }) {
    ...
    useEffect(() => {
        ...
        if (authState.status === "initializing")
            return;
        if (authState.status === "migration-pending")
            return;
        if (authState.status === "unauthenticated")
            return;
        dispatch({ type: "FETCH_PANELS_START" });
        const unsubscribe = panelsService.subscribeToHomePanel((panel) => {
    ...
    const currentPanelId = state.status === "panel" ? state.currentPanel.id : undefined;
    ...
    const loadSubPanels = useCallback(async (parentId, cancelToken) => {
    ...
    const findByRef = useCallback(async (ref) => {
        ...
        if (collectionId !== "panels" && collectionId !== "shared") {
            dispatch({
                ...
                payload: notFoundErr(`Referencia con colección padre desconocida: ${collectionId}`),
            });
            return undefined;
        }
    ...
    // ─── findBySharedId ───────────────────────────────────────────────────────
    const findBySharedId = useCallback(async (sharedId) => {
    ...
    const fetchHomePanel = useCallback(async () => {
        dispatch({ type: "FETCH_PANELS_START" });
    ...
    const findArchived = useCallback(async (parentRef) => {
    ...
    // ─── createPanel ──────────────────────────────────────────────────────────
    const createPanel = useCallback(async (data, opt) => {
        if (!opt) {
        ...
        return err(unexpectedErr("No hay un currentPanel seleccionado para añadir el sub-panel"));
    }, [panelsService, currentPanelId]);
    const archivePanel = useCallback(async (id) => {
    ...
    const unarchivePanel = useCallback(async (id) => {
    ...
    // ─── fetchSubPanels ───────────────────────────────────────────────────────
    const fetchSubPanels = useCallback(async (parentId) => {
    ...
    // ─── deletePanelCascade ─────────────────────────────────────────────────
    const deletePanelCascade = useCallback(async (id) => {
    ...
    const deletePanelArchive = useCallback(async (id) => {
    ...
    return (_jsx(PanelsContext.Provider, { value: value, children: children }));
}
