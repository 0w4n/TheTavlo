...
import type { ClassSlot, ClassSlotContent } from "./classSlot.entity";
 ...
 */
export function resolveWeek(input: ResolveWeekInput): ResolvedClassInstance[] {
  ...
  const instances: ResolvedClassInstance[] = [];

  for (const slot of coveringSlots) {
    const subject = subjectsById.get(slot.subjectId);
    if (!subject) continue; // asignatura archivada/borrada de forma inconsistente — no romper el render (EC-7)

    const date = weekNumberAndDayToDate(schedule, week, slot.dayOfWeek);
    if (isHoliday(schedule, date)) continue; // el festivo gana siempre, EC-2
 ...
 * no romper el render.
 */
function latestCoveringSlotPerGroup(slots: ClassSlot[], week: number): ClassSlot[] {
  const bestByGroup = new Map<string, ClassSlot>();

  for (const slot of slots) {
    if (slot.validFromWeek > week || slot.validToWeek < week) continue;
...
function indexActiveExceptionsByDateAndSlot(
  exceptions: OccurrenceException[],
): Map<string, OccurrenceException> {
  const byKey = new Map<string, OccurrenceException>();
...
function buildInstance(params: {
  ...
  subject: Subject;
  date: Date;
  exception: OccurrenceException | undefined;
  ...
  const content: ClassSlotContent = exception?.overrides
...
export function detectConflicts(
  instances: ResolvedClassInstance[],
): ResolvedClassInstance[] {
  const byDay = new Map<number, ResolvedClassInstance[]>();
  for (const instance of instances) {
    if (instance.isCancelled) continue; // una clase cancelada no puede "solaparse" con nada
    const list = byDay.get(instance.dayOfWeek) ?? [];
    list.push(instance);
    byDay.set(instance.dayOfWeek, list);
  }

  const conflictingKeys = new Set<ResolvedClassInstance>();

  for (const dayInstances of byDay.values()) {
    const sorted = [...dayInstances].sort((a, b) => a.startMinute - b.startMinute);
    for (let i = 0; i < sorted.length; i++) {
      for (let j = i + 1; j < sorted.length; j++) {
        if (sorted[j].startMinute >= sorted[i].endMinute) break; // ya ordenado, no puede haber más solapes desde aquí
  ...
  return instances.map((instance) =>
    conflictingKeys.has(instance) ? { ...instance, hasConflict: true } : instance,
  );
}
