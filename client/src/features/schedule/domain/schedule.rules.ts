...
export default class ScheduleRules {
  ...
  static validateClassSlotContent(
    content: Pick<ClassSlotContent, "startMinute" | "endMinute">,
  ): string | null {
  ...
  static validateSlotCoversWeek(
    slot: { validFromWeek: number; validToWeek: number },
    week: number,
  ): string | null {
  ...
  static validateWeekRangeScope(scope: EditScope): string | null {
    if (scope.kind !== "weekRange") return null;
