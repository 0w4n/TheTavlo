...
export interface OccurrenceException {
  id: string;
  slotGroupId: string; // a qué clase (grupo) afecta
  scheduleId: string;
  date: Timestamp; // fecha exacta (medianoche local), no nº de semana
  kind: ExceptionKind;
  ...
  reason?: string; // "Aula cambiada por obras", opcional, visible en el detalle
  createdAt: Timestamp;
}

export type CreateOccurrenceExceptionDTO = Omit<
  OccurrenceException,
