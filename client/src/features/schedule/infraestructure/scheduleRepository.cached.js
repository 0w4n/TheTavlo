...
import { getCachedActiveSlotVersion, getCachedSchedule, getCachedSlotById, setCachedAttendance, setCachedExceptions, setCachedSchedule, setCachedSlots, setCachedSubjects, upsertCachedAttendance, upsertCachedException, upsertCachedSlot, upsertCachedSubject, } from "./scheduleCache";
/**
 ...
 */
export class CachedScheduleRepository {
    constructor(inner, cacheKey) {
    ...
    async findSchedule() {
        ...
        if (cached !== undefined)
            return { success: true, value: cached ?? undefined };
        const result = await this.inner.findSchedule();
        if (isOk(result))
            setCachedSchedule(this.cacheKey, result.value ?? null);
        return result;
    }
    ...
    async createSchedule(data) {
        ...
        if (isOk(result))
            setCachedSchedule(this.cacheKey, result.value);
        return result;
    }
    ...
    async createSubject(scheduleId, data) {
        ...
        if (isOk(result))
            upsertCachedSubject(this.cacheKey, result.value);
        return result;
    }
    ...
    async createClassSlot(scheduleId, data) {
        ...
        if (isOk(result))
            upsertCachedSlot(this.cacheKey, result.value);
        return result;
    }
    ...
    async upsertAttendance(scheduleId, data) {
        ...
        if (isOk(result))
            upsertCachedAttendance(this.cacheKey, result.value);
        return result;
    }
}
