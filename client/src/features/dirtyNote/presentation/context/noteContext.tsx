...
import type { CreateDirtyNoteDTO } from "#features/dirtyNote/domain/DirtyNote.entity";
...
export function NotesProvider({ children, notesService }: NotesProviderProps) {
  ...
  const createNote = async (noteData: { title: string; body: string }) => {
    try {
      ...
      const createPayload: CreateDirtyNoteDTO = {
        ...noteData,
        createdAt: now,
        updatedAt: now,
      };
      const DirtyNote = await notesService.createNote(createPayload);
      dispatch({ type: "CREATE_NOTE_SUCCESS", payload: DirtyNote });
      await fetchNotes();
    } catch (error) {
      const appErr = unexpectedErr(
  ...
  const updateNote = async (
    ...
    try {
      const DirtyNote = await notesService.updateNote(id, noteData);
      dispatch({ type: "UPDATE_NOTE_SUCCESS", payload: DirtyNote });
      await fetchNotes();
    } catch (error) {
      const appErr = unexpectedErr(
  ...
  const selectNote = (DirtyNote: { id: string; title: string; body: string; createdAt: any; updatedAt: any }) => {
    dispatch({ type: "SELECT_NOTE", payload: DirtyNote as any });
  };
