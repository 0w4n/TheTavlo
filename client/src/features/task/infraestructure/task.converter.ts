...
import type { AnyTask, NodeTask, Task } from "../domain/task.entity";
 ...
 * `NodeTask` de verdad.
 */
export const taskConverter: FirestoreDataConverter<AnyTask> = {
  toFirestore(task: WithFieldValue<AnyTask> | PartialWithFieldValue<AnyTask>): DocumentData {
    const { id: _id, ...data } = task as AnyTask;
    return data;
  },

  fromFirestore(snapshot: QueryDocumentSnapshot, options: SnapshotOptions): AnyTask {
