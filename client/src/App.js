...
import { clearPanelsCache, getPanelsCacheKey, } from "#features/panels/infraestructure/panelsCache";
...
function isLegacyAppPath(pathname) {
    return LEGACY_APP_PREFIXES.some((prefix) => pathname === prefix || pathname.startsWith(`${prefix}/`));
}
export default function App() {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(AuthenticatedApp, {}), _jsx(SpeedInsights, {}), _jsx(Analytics, {})] }));
}
function AuthenticatedApp() {
    ...
    return (_jsx(AnnouncerProvider, { children: _jsx(AuthProvider, { authService: authService, children: _jsx(RouterProvider, { router: appRouter }) }) }));
}
export function ProtectedLayout() {
    ...
    switch (state.status) {
        ...
        case "migration-pending":
            return _jsx(MigrationDialog, {});
        case "error":
            return _jsx("div", { children: state.error });
        case "authenticated":
            return _jsx(AuthenticatedLayout, { user: state.user });
    }
}
function AuthenticatedLayout({ user }) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    const cacheKey = useMemo(() => getPanelsCacheKey(user), [user.accountType, user.id]);
    const panelsService = useMemo(() => {
        const repository = new FirebasePanelsRepository(firebaseService.firestore, () => user);
    ...
    return (_jsx(PanelsProvider, { panelsService: panelsService, children: _jsx(GlobalContextProvider, { children: _jsx(ProviderApp, {}) }) }));
}
function ProviderApp() {
    const globalContext = useGlobalContext();
    console.log("GlobalContext:", globalContext);
    const widgetService = useMemo(() => {
        const widgetRepository = new FirebaseWidgetRepository(firebaseService.firestore, () => globalContext);
        return new WidgetService(widgetRepository);
    }, [globalContext]);
    const invitationService = useMemo(() => {
        const invitationRepository = new FirebaseInvitationRepository(firebaseService.firestore, () => globalContext);
        return new InvitationService(invitationRepository);
    }, [globalContext]);
    const eventService = useMemo(() => {
        const eventRepository = new FirebaseEventRepository(firebaseService.firestore, () => globalContext);
        return new EventsService(eventRepository);
    }, [globalContext]);
    const taskService = useMemo(() => {
        const taskRepository = new FirebaseTaskRepository(firebaseService.firestore, () => globalContext);
        return new TasksService(taskRepository);
    }, [globalContext]);
    return (_jsx(_Fragment, { children: _jsx(WidgetsProvider, { widgetService: widgetService, children: _jsx(EventsProvider, { eventsService: eventService, children: _jsx(InvitationProvider, { invitationService: invitationService, children: _jsx(TasksProvider, { tasksService: taskService, children: _jsx(Outlet, {}) }) }) }) }) }));
}
