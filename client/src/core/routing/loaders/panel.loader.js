...
function defaultDeps() {
    return {
        getCurrentUser,
        createPanelsService: (user) => new PanelsService(new CachedPanelsRepository(new FirebasePanelsRepository(firebaseService.firestore, () => user), getPanelsCacheKey(user))),
    };
}
/**
 ...
 */
export function createPanelLoader(deps = defaultDeps()) {
    /**
     ...
     */
    return async function panelLoader({ params, }) {
        ...
        if (!parsed.ok) {
            ...
            // el modal correcto.
            const modal = parsed.reason === "no-panel" ? "task-needs-panel" : "invalid-url";
