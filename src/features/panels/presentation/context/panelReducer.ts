...
type PanelsAction =
  ...
  | {
      ...
      payload: { parentId: string; panels: Panel[] };
    }
  | { type: "FETCH_SUBPANELS_ERROR"; payload: AppErr }
...
function reconcileSubPanels(
  ...
  if (belongsToCurrent) {
    if (existingIndex === -1) return [...subPanels, mutated];
  ...
  if (existingIndex === -1) return subPanels;
...
export function panelsReducer(
  ...
  switch (action.type) {
    ...
    case "FETCH_SUBPANELS_SUCCESS": {
      ...
      if (state.currentPanel.id !== action.payload.parentId) return state;

      console.log("FETCH", action.payload.panels);
    ...
    case "CREATE_PANEL_SUCCESS": {
      ...
      console.log("CREATE", state.subPanels, action.payload);
    ...
    case "UPDATE_PANEL_SUCCESS": {
      ...
      const currentPanel =
        state.currentPanel.id === updated.id ? updated : state.currentPanel;

      return {
        ...state,
        selectedPanel:
          state.selectedPanel?.id === updated.id
            ? updated
            : state.selectedPanel,
        currentPanel,
        subPanels: reconcileSubPanels(
