...
describe("getPanelsCacheKey", () => {
  it("combina accountType e id — namespacea por usuario", () => {
    expect(getPanelsCacheKey({ accountType: "users", id: "abc" })).toBe(
      "users/abc",
    );
    expect(getPanelsCacheKey({ accountType: "guests", id: "abc" })).not.toBe(
      getPanelsCacheKey({ accountType: "users", id: "abc" }),
    );
  });
});
