 ...
 */
export function parsePanelPath(splat: string | undefined): ParsePanelPathResult {
  ...
  switch (tail.length) {
    case 0:
      ...
      break;
    case 1:
      if (tail[0] === "task") view = { type: "task-list" };
      else if (tail[0] === "calendar") view = { type: "calendar" };
      else return { ok: false, reason: "malformed" };
      break;
    case 2:
      if (tail[0] === "task") view = { type: "task-detail", taskId: tail[1] };
      else if (tail[0] === "calendar") view = { type: "event-detail", eventId: tail[1] };
      else return { ok: false, reason: "malformed" };
      break;
    default:
...
export function buildPanelPath(
  panelIds: string[],
  view: PanelView = { type: "dashboard" },
): string {
  if (panelIds.length === 0) {
    throw new Error(
      "buildPanelPath requiere al menos un panelId. " +
...
export function activePanelId(path: PanelPath): string {
